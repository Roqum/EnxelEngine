...
namespace Enxel
{
	...
	private:
		void GreedyMeshing(std::vector<Vertex>& vertices, std::vector<uint32_t>& indices, std::vector<bool> mask, const CubeFace& faceSide);
	...
	inline void toCoords(int idx, int& x, int& y, int& z) {
		...
		z = (idx >> m_Settings.ChunkShiftZ) & m_Settings.ChunkMask;
	}
